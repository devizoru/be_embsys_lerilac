...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    int fd_blue = open("/sys/class/gpio/export", O_WRONLY);
    if (fd_blue == -1) {
        perror("Unable to open /sys/class/gpio/export");
        exit(1);
    }

    if (write(fd_blue, "24", 2) != 2) {
    ...
    int fd_red = open("/sys/class/gpio/export", O_WRONLY);
    if (fd_red == -1) {
        perror("Unable to open /sys/class/gpio/export");
        exit(1);
    }

    if (write(fd_red, "23", 2) != 2) {
        perror("Error writing to /sys/class/gpio/export");
        exit(1);
    }
    ...
    int fd_green = open("/sys/class/gpio/export", O_WRONLY);
    if (fd_green == -1) {
        perror("Unable to open /sys/class/gpio/export");
        exit(1);
    }

    if (write(fd_green, "18", 2) != 2) {
        perror("Error writing to /sys/class/gpio/export");
        exit(1);
    }
    ...
    fd_blue = open("/sys/class/gpio/gpio24/direction", O_WRONLY);
    if (fd_blue == -1) {
        perror("Unable to open /sys/class/gpio/gpio24/direction");
        exit(1);
    }

    if (write(fd_blue, "out", 3) != 3) {
    ...
    fd_blue = open("/sys/class/gpio/gpio24/value", O_WRONLY);
    if (fd_blue == -1) {
        perror("Unable to open /sys/class/gpio/gpio24/value");
        exit(1);
    }
    
    fd_red = open("/sys/class/gpio/gpio23/direction", O_WRONLY);
    if (fd_red == -1) {
        perror("Unable to open /sys/class/gpio/gpio24/direction");
        exit(1);
    }

    if (write(fd_red, "out", 3) != 3) {
    ...
    fd_red = open("/sys/class/gpio/gpio23/value", O_WRONLY);
    if (fd_red == -1) {
        perror("Unable to open /sys/class/gpio/gpio23/value");
        exit(1);
    }
    
    fd_green = open("/sys/class/gpio/gpio18/direction", O_WRONLY);
    if (fd_green == -1) {
        perror("Unable to open /sys/class/gpio/gpio18/direction");
        exit(1);
    }

    if (write(fd_green, "out", 3) != 3) {
    ...
    fd_green = open("/sys/class/gpio/gpio18/value", O_WRONLY);
    if (fd_green == -1) {
    ...
    while (1)
    {
        ...
        if (write(fd_blue, "1", 1) != 1) {
            perror("Error writing to /sys/class/gpio/gpio24/value");
            exit(1);
        }
        usleep(50000);

        if (write(fd_blue, "0", 1) != 1) {
            perror("Error writing to /sys/class/gpio/gpio24/value");
            exit(1);
        }
        usleep(50000);
        
        if (comp_data.temperature > 20) {
        
            if (write(fd_red, "1", 1) != 1) {
                perror("Error writing to /sys/class/gpio/gpio24/value");
                exit(1);
            }
            if (write(fd_green, "0", 1) != 1) {
                perror("Error writing to /sys/class/gpio/gpio24/value");
                exit(1);
            }
        }
        ...
        {
            if (write(fd_green, "1", 1) != 1) {
                perror("Error writing to /sys/class/gpio/gpio24/value");
                exit(1);
            }
            if (write(fd_red, "0", 1) != 1) {
                perror("Error writing to /sys/class/gpio/gpio24/value");
                exit(1);
            }
    ...
    fd_blue = open("/sys/class/gpio/unexport", O_WRONLY);
    if (fd_blue == -1) {
